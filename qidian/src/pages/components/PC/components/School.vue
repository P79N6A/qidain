		...
		<div class="search" >
			...
			<div class="scroll" ref='wapper'>
				<ul>
					...
					<li @click="fn(item.name,item.schoolId)" v-for="item in schoolList">{{item.name}}大学</li>
				</ul>
			</div>
		</div>
	</div>
...
//					console.log(res)
				...
				this.$router.push({name:'BeAgent',params:{name:this.name,phone:this.phone,province:province}})
			},
			getSchool(){
				axios({
					headers:{'Content-Type':'application/x-www-form-urlencoded'},
...
//				 $.post({url:"http://172.16.10.181:8080/Campus2019/schoolManage/getSchoolSelect",success:(result)=>{
//			        console.log(result.rows)
//			        this.schoolList=result.rows
//			    }});
			},
			search(){
