	...
	<div class="details">
		<div class="cont">
			<div class="agentMsg" >
				...
				<div>
					<h2>{{msg.xm}}</h2>
					<p>{{msg.registrationTime}}</p>
				</div>
			</div>
			<div class="agentBenefits">
				<p><em>{{msg.cardNum}}</em><br /><span>办卡数</span></p>
				<p class="middleP"><em>{{msg.uploadNum}}</em><br /><span>已上传截图</span></p>
				<p><em>{{msg.throughNum}}</em><br /><span>已审核通过截图</span></p>
			</div>
			<div class="memberList">
				<ul>
					<li v-for="item of data">
						<p><span>{{msg.xm}}</span><span class="tel">{{msg.mobile}}</span></p>
						<div>
							<p><span>上传截图:</span><img :src="item.img|img"/></p>
							<p class="pass"><span>审核状态:<i>{{state|state}}</i></span><button :class="item.receiveState==0?'btn1':'btn2'" v-if='state==2' @click="getBonus(item.applyId)">{{txt}}</button></p>
						</div>
					</li>
				</ul>
			</div>
